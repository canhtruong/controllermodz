 ...
 * Class Save
 *
 * @package Bss\GiftCard\Controller\Adminhtml\Pattern\FileUploader
 */
        ...
        if ($importCodestRawData[0][0] != 'code' &&
            ...
            try {
                ...
                if (!empty($importCodestRawData)) {
                    ...
                    if ($data['error']) {
                        $message .= __(
                            '%1 code(s) bring errors because of duplicate data or empty row or Code is incorrect format. Please check again.',
                            $data['error']
                        );
                    } else {
