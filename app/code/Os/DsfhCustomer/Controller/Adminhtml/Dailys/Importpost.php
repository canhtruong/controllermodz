                ...
                if (pathinfo($files['filename']['name'], PATHINFO_EXTENSION) == 'xls') {
                    ...
                    */
                    ...
                    foreach($data as $value){
                        if($ii == 0){ $ii = 1; } else {
                            $query = "INSERT INTO `dsfh_customer` (`DSFH_CUSTOMER_ID`, `DSFH_REQUEST_ID`, `ONHAND_DATE`, `CUSTOMER_ID`, `CUSTOMER_NUMBER`, `CUSTOMER_NAME`, `CUSTOMER_ORGANIZATION_ID`, `CUSTOMER_ITEM_ID`, `CUSTOMER_ITEM_NUMBER`, `CUSTOMER_ITEM_DESCRIPTIOA`, `PRIMARY_UOM_CODE`, `SUBINVENTORY`, `LOT`, `ORIGINATION_DATE`, `EXPIRY_DATE`, `QTY`, `STATUS`, `CREATION_DATE`, `CREATION_BY`, `LAST_UPDATE_DATE`, `LAST_UPDATE_BY`, `SEQID`, `ORG_ID`) VALUES (NULL, '', '".str_replace("/", "-", $value[0])."', '', '', '', '', '".$value[1]."', '".$value[2]."', '".$value[3]."', '".$value[4]."', '".$value[5]."', '".$value[6]."', '".str_replace("/", "-", $value[7])."', '".str_replace("/", "-", $value[8])."', '".$value[9]."', '".$value[10]."', '', '', '', '', '', ''); ";
                            $result = $connection->query($query);
                        }
                    }
                    $this->messageManager->addSuccess('All items are imported !!!');
